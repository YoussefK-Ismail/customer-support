...
def check_ticket_status(ticket_id: str) -> str:
    ...
    return json.dumps(tickets.get(ticket_id.strip().upper(), {"error": f"Ticket not found. Valid: TKT-001 to TKT-004"}))
...
def build_agent(api_key, product_filter):
...
1. Answer using the knowledge base context provided
2. Give clear numbered troubleshooting steps
3. Use tools when needed (tickets, upgrades)
...
def main():
    ...
    with st.sidebar:
        ...
        for q in ["My CloudStore sync stopped working", "How do I set up SSO for SecureVault?",
                  "DataFlow dashboard is loading slowly", "I got a breach alert, what should I do?",
                  "Check status of ticket TKT-002", "Upgrade options for CloudStore Pro?"]:
    ...
    if user_input:
        ...
        with st.chat_message("assistant", avatar="🎧"):
            with st.spinner("🤔 Analyzing your issue…"):
                try:
                    output   = build_agent(api_key, product_filter).invoke({
